...
func main() {
	f, err := os.OpenFile("redis_no_ttl.csv", os.O_RDWR|os.O_CREATE|os.O_APPEND, 0666)
	if err != nil {
...
func iterator(iter *redis.ScanIterator, ctx context.Context, ch chan string) {
	for iter.Next(ctx) {
		ch <- iter.Val()
	}
...
func del(ctx context.Context, rdb *redis.Client, in <-chan string) error {
	...
	for key := range in {
		memoryUsage := strconv.FormatInt(rdb.MemoryUsage(ctx, key).Val(), 10)
		log.Println(key + "," + memoryUsage)
		pipe.Del(ctx, key)
...
func logdata(ctx context.Context, rdb *redis.Client, in <-chan string) error {
	for key := range in {
		memoryUsage := strconv.FormatInt(rdb.MemoryUsage(ctx, key).Val(), 10)
		log.Println(key + "," + memoryUsage)
	}
